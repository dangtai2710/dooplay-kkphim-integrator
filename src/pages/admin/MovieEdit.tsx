  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (formData.directors !== undefined) {
        ...
        const directorNames = formData.directors.split(",").map((n: string) => n.trim()).filter(Boolean);
        ...
        for (const name of directorNames) {
          ...
          if (director) {
            await supabase.from("movie_directors").insert({
              movie_id: movieId,
      ...
      // Update actors
      if (formData.actors !== undefined) {
        await supabase.from("movie_actors").delete().eq("movie_id", movieId!);
        const actorNames = formData.actors.split(",").map((n: string) => n.trim()).filter(Boolean);
      ...
      if (formData.tags !== undefined) {
        ...
        const tagNames = formData.tags.split(",").map((n: string) => n.trim()).filter(Boolean);
